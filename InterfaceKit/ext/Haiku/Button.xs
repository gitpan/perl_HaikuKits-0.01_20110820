...
MODULE = Haiku::InterfaceKit	PACKAGE = Haiku::Button

PROTOTYPES: DISABLE

SV*
BButton::new(frame, name, label, message, ...)
	INPUT:
		BRect frame;
		const char* name;
		const char* label;
	...
	CODE:
		...
		retval = new BButton(frame, name, label, message, resizingMode, flags);
...
newWithoutFrame(CLASS, name, label, message, ...)
	...
	CODE:
		...
		retval = new BButton(name, label, message, flags);
...
newBareBones(CLASS, label, ...)
	...
	CODE:
		...
		retval = new BButton(label, message);
...
DESTROY(perl_obj)
	...
	CODE:
		DEBUGME(2,"Deleting perl wrapper for BButton");
		DUMPME(1,perl_obj);
		link = get_link_data(perl_obj);
		if (link != NULL && ! PL_dirty && link->can_delete_cpp_object) {
			DEBUGME(2,"-->Deleting the wrapped c++ object");
			cpp_obj = (BButton*)link->cpp_object;
			delete cpp_obj;
...
SV*
BButton::ResolveSpecifier(message, index, specifier, form, property)
	INPUT:
		BMessage* message;
		int32 index;
		BMessage* specifier;
...
bool
BButton::GetSupportedSuites(data)
	INPUT:
		BMessage* data;
	INIT:
		status_t retval;
...
bool
BButton::operator_eq(object, swap)
	INPUT:
		BButton* object;
		IV swap;
	OVERLOAD: ==
...
bool
BButton::operator_ne(object, swap)
	INPUT:
		BButton* object;
		IV swap;
	OVERLOAD: !=
...
BOOT:
	sv_setsv(
		get_sv("Haiku::Button::()", TRUE),
		&PL_sv_yes	// so we don't get fallback errors
	);
    newXS("Haiku::Button::()", XS_Haiku__InterfaceKit_nil, file);
