...
MODULE = Haiku::InterfaceKit	PACKAGE = Haiku::SeparatorItem
...
BSeparatorItem::new()
	INIT:
		BSeparatorItem* retval;
		SV* retval_sv = newSV(0);	// retcount == 1
	CODE:
		// item 0: CLASS
		
		retval = new BSeparatorItem();
...
newFromArchive(CLASS, archive)
	...
	CODE:
		...
		retval = new BSeparatorItem(archive);
...
DESTROY(perl_obj)
	INPUT:
		SV* perl_obj;
	INIT:
		BSeparatorItem* cpp_obj;
		object_link_data* link;
	CODE:
		DEBUGME(2,"Deleting perl wrapper for BSeparatorItem");
		DUMPME(1,perl_obj);
		link = get_link_data(perl_obj);
		if (link != NULL && ! PL_dirty && link->can_delete_cpp_object) {
			DEBUGME(2,"-->Deleting the wrapped c++ object");
			cpp_obj = (BSeparatorItem*)link->cpp_object;
			delete cpp_obj;
...
SV*
BSeparatorItem::Instantiate(data)
	INPUT:
		BMessage* data;
	INIT:
		BArchivable* retval;
...
void
BSeparatorItem::SetEnabled(enabled)
	INPUT:
		bool enabled;
	CODE:
		// item 0: THIS
...
bool
BSeparatorItem::operator_eq(object, swap)
	INPUT:
		BSeparatorItem* object;
		IV swap;
	OVERLOAD: ==
	CODE:
		RETVAL = THIS == object;
...
bool
BSeparatorItem::operator_ne(object, swap)
	INPUT:
		BSeparatorItem* object;
		IV swap;
	OVERLOAD: !=
...
BOOT:
	sv_setsv(
		get_sv("Haiku::SeparatorItem::()", TRUE),
		&PL_sv_yes	// so we don't get fallback errors
	);
    newXS("Haiku::SeparatorItem::()", XS_Haiku__InterfaceKit_nil, file);
